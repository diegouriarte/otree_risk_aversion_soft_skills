...
Filtramos los datos correspondientes a la sesión realizada, con participantes que han llegado hasta la última etapa
...
Vemos que se cuentan con `r nrow(data_filtrada)` datos de personas que han terminado el experimento.Se realizarán modificaciones sobre este 
data set filtrado.
...
* Extraversión: 1 + 6R ingreso_datos.1.player.Q1
* Amabilidad: 2R + 7
...
Ahora, calculemos los valores promedio y las desviaciones estándar para cada rasgo de personalidad:
...
## Calculando coeficientes de aversión al riesgo de según BRET

Las variables a ser utilizadas son `bret.n.player.boxes_collected` donde 'n' es el número del juego utilizado.
Primero, realizaremos el cálculo con el promedio de cajas recolectadas por cada participante:
...
El coeficiente de aversión al riesgo considerando una función CRRA se calcula como $r = \frac{100-2k}{k+100}$, dode k es el número de
cajas recolectadas en promedio (o máxima):
...
## Calculando coeficientes de aversión al riesgo de según loterías

Se determinará el coeficiente de aversión al riesgo 
...
Ahora grafiquemos la relación entre las dos formas de medición del riesgo:
...
```
En adición, se grafica un dot_plot del valor de r lotería elegida versus BRET.
...
## Efecto de las habilidades blandas en la aversión al riesgo

Se realizará una regresión lineal controlando por la edad y el cuadrado de la edad, así como por el sexo.
...
Ahora, se realizarán las gráficas de puntos para los factores de personalidad versus valores de r.
